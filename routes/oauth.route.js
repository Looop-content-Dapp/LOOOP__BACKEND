...
OAuthRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        VALID_OAUTH_PROVIDERS.includes(provider) === false ||
        !looopAuthToken
      ) {
        return res
          .status(400)
          .json({
            ...
            message: "Provide Valid Provider & Auth",
          });
      }
      ...
      if (!oauthToken) {
        return res
          ...
          .json({
            error: "Invalid Request",
            message: "No OAuth2 payload found in body",
          });
      }
      ...
      return res
        .status(200)
        .json({ status: true, message: "Third-party OAUth saved" });
    } catch (error) {
      return res
        .status(500)
        .json({ error: "Unexpected Error Occurred", message: String(error) });
    }
  }
);
